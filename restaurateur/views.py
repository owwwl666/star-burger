...
from foodcartapp.services.services_location import annotate_coordinates
from foodcartapp.services.services_location import calculate_distance
...
def view_orders(request):
    ...
    products_in_restaurant = {
        restaurant: restaurant.menu_items.values_list("product", flat=True)
        for restaurant in restaurants
    }.items()
    products_in_order = {
        order: order.ordered_products.values_list("product", flat=True)
        for order in orders
    }.items()
    ...
    return render(
        ...
        context={"order_items": order_details, "url": request.path},
    )
